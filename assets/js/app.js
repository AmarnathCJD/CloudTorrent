    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach(t => {
                ...
                html += `<div class="w-full sm:w-1/2 md:w-1/4 xl:w-1/5 p-1">
                <div class="block rounded-lg shadow-lg bg-white max-w-sm text-center border-gray-300 ">
                    <div class="py-3 px-6 border-b border-gray-300">
                    <a style='margin-top: 10px;' href="#" class="flex items-center p-3 text-base font-bold text-gray-900 bg-gray-50 rounded-lg hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                    ${svg}
                   <span class="flex-1 ml-3 text-sm whitespace-nowrap">${t.speed}</span>
                    <span class="inline-flex items-center justify-center px-2.5 py-0.5 ml-3 text-xs font-medium text-gray-500 bg-gray-200 rounded dark:bg-gray-700 dark:text-gray-400">${t.eta}</span>
                </a>
                    </div>
                    <div class="p-6 h-36">
                        <h5 class="text-gray-900 text-sm font-medium mb-2">${TrimString(t.name)} <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded text-pink-600 bg-pink-200 uppercase last:mr-0 mr-1">
                        ${t.size}
                      </span></h5>
                        <p class="text-gray-700 text-base mb-4">

<div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
<div class="bg-${pcolor}-600 h-2.5 rounded-full" style="width: ${t.perc}"></div>
</div>
                    ...
                    class=" inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded-l shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Browse</a>
                    <button type="button"
                    class=" inline-block px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded-r shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Delete</button>
                    </div>
                    </div>
